...
class RoomsInput extends EventEmitter {
  ...
  getContent(roomId, msgtype, message, reply, edit) {
    ...
    const output = settings.isMarkdown ? markdown : plain;
  ...
  async sendInput(roomId, msgType) {
    ...
    if (this.getMessage(roomId).trim() !== '') {
      const content = this.getContent(roomId, msgType, input.message, input.replyTo);
